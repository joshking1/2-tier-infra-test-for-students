...
resource "aws_security_group" "app_db_sg" {
  vpc_id = aws_vpc.main.id
  name   = "AppDBSecurityGroup"

  # Define rules for application and database tier
  ingress {
    from_port   = 5432  # Example: PostgreSQL port
    to_port     = 5432
    protocol    = "tcp"
    security_groups = [aws_security_group.web_sg.id]  # Allow traffic from web tier
  }

  ingress {
    from_port   = 80  # Example: HTTP port
    to_port     = 80
    protocol    = "tcp"
    security_groups = [aws_security_group.web_sg.id]  # Allow traffic from web tier
  }
...
resource "aws_instance" "app_db" {
  ...
  tags = {
    Name = "AppDBInstance-${count.index}"
  }
}
